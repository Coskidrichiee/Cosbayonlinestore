...
function buyButtonClicked() {
    ...
    if (cartContent.children.length > 0) {
        ...
        alert("Your order has been placed! Please fill in the following details.");
        ...
        alert("You have not added any product to the cart.");
...
function removeCartItem(event) {
    ...
    if (cartContent.children.length === 0) {
        ...
        saveCartToLocalStorage(); // Save remaining cart items to local storage
    }
...
function addCartClicked(event) {
    ...
    var price = shopProducts.getElementsByClassName("price")[0].innerText.replace(/₦|,/g, ''); // Remove any currency symbols or commas
...
function loadCartFromLocalStorage() {
    ...
    cartItems.forEach(item => {
        addProductToCart(item.name, item.price, ''); // Placeholder image URL if not available
...
function addProductToCart(title, price, productImg) {
    ...
    // Show the cart count and update it
    var cartCountElement = document.querySelector('.cart-count');
    cartCountElement.style.display = 'inline-block'; // Show the cart count
    var count = parseInt(cartCountElement.textContent);
...
// scroll to top button
    ...
    if (window.location.pathname.endsWith('index.php') || window.location.pathname.endsWith('index')) {
        alert("WELCOME TO COSBAY ONLINE STORE!");
    }
